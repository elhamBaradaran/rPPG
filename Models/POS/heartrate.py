...
# Heart-rate band, in cycles per second (Hz).
# 0.7 Hz = 42 BPM (low), 4.0 Hz = 240 BPM (high). Keeps only plausible pulse.
LOW_HZ = 0.7
HIGH_HZ = 4.0
...
def bandpass_filter(signal, fps, low, high):
    ...
    nyquist = 0.5 * fps          # highest frequency the fps can represent
    low_cut = low / nyquist
    high_cut = high / nyquist
    # A 3rd-order Butterworth bandpass. b, a describe the filter.
    b, a = butter(3, [low_cut, high_cut], btype='band')
    # filtfilt applies it forward and backward (no time shift).
    return filtfilt(b, a, signal)


def estimate_bpm(pulse, fps):
    """Find the dominant frequency via FFT and convert to BPM."""
    n = len(pulse)
    # FFT gives the strength of each frequency in the signal.
    fft = np.abs(np.fft.rfft(pulse))
    freqs = np.fft.rfftfreq(n, d=1.0 / fps)     # the frequency for each bin

    # Only look within the heart-rate band.
    valid = (freqs >= LOW_HZ) & (freqs <= HIGH_HZ)
    if not np.any(valid):
        return 0.0
    # The strongest frequency in that band = heart rate.
    peak_freq = freqs[valid][np.argmax(fft[valid])]
    return peak_freq * 60.0                      # Hz -> beats per minute


print("Measuring heart rate. Hold still, look at the camera. Press 'q' to quit.")
...
while True:
    ...
    if len(g_buffer) == BUFFER_SIZE:
        ...
        cv2.putText(wave_img, "Filtered pulse", (10, 20),
    ...
    # Show the big BPM number on the camera window.
